...
public final class TypeAwareSynonymFilter extends TokenFilter {
  ...
  private List<String> allowedTypes = Arrays.asList(new String[]{"jj", "np", "nn", "vb",
          "od", "vbg", "vbz", "nns", "nps"});
    ...
    while (true) {
      ...
      while (inputSkipCount != 0) {
        ...
        if (!input.consumed && (input.keepOrig || !input.matched)) {
          ...
          return true;
        } else if (outputs.upto < outputs.count && Collections.binarySearch(allowedTypes, typeAtt.type()) > 0) {
